...
class CategoryController extends Controller
{
    ...
    public function update(Request $request){
        ...
        $isExistName=Category::whereName($request->name)->whereNotIn('id',[$request->id])->first();
        $isExistSlug=Category::whereSlug(Str::slug($request->slug))->whereNotIn('id',[$request->id])->first();
        if($isExistName or $isExistSlug){
            toastr()->error($request->name.' adında bir kategori zaten mevcut','Dikkat');
            return \redirect()->back();
        }
        // return $request->id.'<br>'.$request->name.'<br>'.$request->slug;
        // die;
    ...
    public function delete(Request $request){
        ...
        if($count>0){
            ...
            $message='Bu kategoriye ait <strong>'.$count.'</strong> makale <strong>'.$defaultCategory->name.'</strong> kategorisine aktarıldı.';
